...
<script>
  ...
  totalSample ++;
  appendnDropzoneDom(totalSample)
  dropzone_instance[totalSample] = initDropzoneExp('dropzone_exp' + totalSample, totalSample)
  $('.dz-message').css({
    'font-size': '18px'
  })
}
function remove_row(item) {
  ...
  removeDropzoneDom(totalSample)
  totalSample --;
  detectEnableSubmit()
}

function appendnDropzoneDom(sampleIndex){
  $('#dropzoneExpDiv').append(
      $('<div>', {
        id: "dropzone_option" + sampleIndex,
        class: 'dropzone_exp col-sm-2 dropzone border-grey rounded dz-clickable'
      }).html(
        '<span>Sample name: </span><input name="dropzone_sample_name'+sampleIndex+'" type="text" id="dropzone_sample_name'+sampleIndex+'" size="20" value="sample'+sampleIndex+'"/> <button type="button" id="reset_btn_'+sampleIndex+'" data-tag="'+sampleIndex+'" class="btn btn-light" onclick="resetCurrentRow(this);" style="margin-top:40px">Reset row</button></div>'
      )
  )
    $('#dropzoneExpDiv').append(
      $('<div>', {
        id: "dropzone_exp" + sampleIndex,
        class: 'col-sm-9 dropzone border-grey rounded dz-clickable dropzone_exp'
      }).html(
        "</div>"
      )
    )
...
function removeDropzoneDom(sampleIndex){
  $("#dropzone_option" + sampleIndex).remove(
    )
  $("#dropzone_exp" + sampleIndex).remove(
  )
  if(exp_file_status.length > 2) {
    exp_file_status.pop()
  }
}

function resetCurrentRow(item){
  var this_sample_index = $('#'+item.id).data('tag')
  var myDropzone = Dropzone.forElement('#dropzone_exp'+this_sample_index);
...
function initDropzoneExp(id,sampleIndex) {
    this_dropzone_instance = $('#'+id).dropzone({
      dictDefaultMessage:
      'Sample'+ sampleIndex + '. Drag or click to upload your gene expression matrix, supported format:  <br>1. HDF5 feature barcode batrix (.h5).<br>2. Gene-barcode matrices (3 gzip files in your 10X output directory). ',
    acceptedFiles: '.gz,.zip,.h5,.hdf5,.zip',
    url: 'upload_integration.php',
    maxFiles: 3,
    parallelUploads: 3,
    ...
    sending: function(file, xhr, formData) {
      ...
      formData.append('sample',sampleIndex)
    ...
    success: function(file, response) {
      ...
      if (response.data[0] === "1") {
        exp_file_status.push(1)
      }
      detectEnableSubmit()
    }
  })
  return this_dropzone_instance
  }
...
$(document).ready(function() {
  ...
  dropzone_instance[1] = initDropzoneExp('dropzone_exp1', 1)
  dropzone_instance[2] =  initDropzoneExp('dropzone_exp2', 2)
  ...
  $('#load_exp').click(function(evt) {
    ...
    $("#integration_method_arg option[value='Seurat']").prop('selected', true)
  ...
  $('select#integration_method_arg').on('change', function(value) {
    detectEnableSubmit()
  })
})
</script>
<main role="main" class="container" style="min-height: calc(100vh - 182px);">
  ...
  <form method="POST" action="{{$URL}}" encType="multipart/form-data" id="needs-validation">
    ...
    <div class="text-center"> <a href="submit.php"><button type="button" class="btn btn-light btn-lg">Single scRNA-seq dataset</button></a>
      <button type="button" class="btn btn-submit btn-lg">Multiple scRNA-seq dataset integration</button></div>
		<h2 class="text-center">Job Submission - Multiple scRNA-seq dataset integration</h2>
		<div class="form-group row">
			<div class="form-check col-sm-12 ">
				<label class="form-check-label" for="expfile">Upload gene expression matrix: <span class="glyphicon glyphicon-question-sign" data-toggle="tooltip" data-original-title="A gene expression file with genes as rows and cells as columns. Users can provide 10X hdf5 or gene-barcode matrices to our server, the file usually named as filtered_feature_bc_matrix.h5 or in the filtered_feature_bc_matrix folder. "> </span>
				</label>
			</div>
...
<div class="form-check col-sm-7 ">
  <label class="form-check-label" for="integration_method_select">Integration method:
  <span class="glyphicon glyphicon-question-sign" data-toggle="tooltip" data-original-title="Specifies the integration method."> </span> 
      </label>
  <select class="selectpicker" id="integration_method_arg" name="integration_method_arg[]" title="Select integration method...">
