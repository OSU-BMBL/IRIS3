...
function make_clust_main(inst_network, root_id) {
  file_size = get_filesize(inst_network, function (size) {
    //8000000
    console.log(
      inst_network + ' heatmap file size: ' + size / 1000000 + ' MB. '
    )
    if (size > 12000000 && window.location.pathname == '/CeRIS/results.php') {
      document.getElementById(root_id.substr(1)).innerHTML =
        "<p>The heatmap has been automatically disabled as page may crash. Click 'Open in new tab' if you would like to check the heatmap for details.</p>"
    } else if (window.location.pathname == '/CeRIS/results_test.php') {
      document.getElementById(root_id.substr(1)).innerHTML =
        "<p>The heatmap has been automatically disabled as page may crash. Click 'Open in new tab' if you would like to check the heatmap for details.</p>"
    } else {
      d3.json(inst_network, function (network_data) {
...
function resize_container(args) {
  ...
  var screen_width = $('.panel-body').width() - 120
